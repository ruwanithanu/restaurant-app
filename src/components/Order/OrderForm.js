...
export default function OrderForm(props) {
    ...
    useEffect(() => {
        createAPIEndpoints(ENDPOINTS.CUSTOMER).fetchAll()
        .then(res => {
    ...
    useEffect(() => {  
        if(orderId == 0)
            ...
            createAPIEndpoints(ENDPOINTS.ORDER).fetchById(orderId)
            .then(res => {
                setValues(res.data);
                setErrors({});
            })
    ...
    const submitOrder = e => {
        ...
        if(validateForm()){
            if(values.orderMasterId == 0){
                createAPIEndpoints(ENDPOINTS.ORDER).create(values)
                .then(res => {
                    resetFormControls();
                    setNotify({
                        isOpen: true,
                ...
                createAPIEndpoints(ENDPOINTS.ORDER).update(values.orderMasterId, values)
                .then(res => {
                    setOrderId(0);
                    setNotify({
                        isOpen: true,
